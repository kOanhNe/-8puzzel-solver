...
def iterative_deepening(start, goal, partial_steps=10):
    ...
    def dfs(state, path, depth_limit, visited):
        ...
        for move_name in MOVES:
            ...
            if new_state and state_to_tuple(new_state) not in visited:
                visited.add(state_to_tuple(new_state))
                result = dfs(new_state, path + [move_name], depth_limit, visited)
                visited.remove(state_to_tuple(new_state))
                if result:
                    return result
        return None
    ...
    while depth < 50 and (partial_steps == float('inf') or depth < partial_steps):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
        if paused:
            ...
            return []
        visited = {state_to_tuple(start)}
        result = dfs(start, [], depth, visited)
        nodes_expanded += len(visited)
...
def steepest_hill_climbing(start, goal, restarts=10, partial_steps=10):
    ...
    for _ in range(restarts):
        ...
        while len(path) < partial_steps and steps_without_improvement < 5:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
            ...
            next_state, move_name = get_best_move(current)
            if not next_state or manhattan_distance(next_state, goal) >= current_h:
                steps_without_improvement += 1
                if steps_without_improvement >= 5:
                    neighbors = [(move(current, m)[0], m) for m in MOVES if move(current, m)[0]]
                    if neighbors:
    ...
    for _ in range(restarts):
        ...
        while len(path) < partial_steps and steps_without_improvement < 5:
            ...
            for move_name in MOVES:
                ...
                if new_state and manhattan_distance(new_state, goal) < current_h:
                    path.append(move_name)
                    current = new_state
                    steps_without_improvement = 0
                    break
    ...
    for _ in range(restarts):
        ...
        while len(path) < partial_steps and steps_without_improvement < 5:
            ...
            if not neighbors:
                steps_without_improvement += 1
                if steps_without_improvement >= 5:
                    all_neighbors = [(move(current, m)[0], m) for m in MOVES if move(current, m)[0]]
                    if all_neighbors:
                        next_state, move_name = random.choice(all_neighbors)
                        path.append(move_name)
                        current = next_state
                        steps_without_improvement = 0
                break
...
def partially_observable(start, goal, partial_steps=10):
    ...
    nodes_expanded = 0
    max_depth = 0
    max_iterations = 500  # Giới hạn số lần lặp
    max_belief_size = 10  # Giới hạn số trạng thái trong belief_state
    ...
    while len(path) < partial_steps and iterations < max_iterations:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
        ...
        # Kiểm tra xem có trạng thái nào trong belief_state đạt mục tiêu không
        for state in belief_state:
            if state_to_tuple(normalize_state(state)) == state_to_tuple(goal):
                statistics.update({
                    "nodes_expanded": nodes_expanded,
                    "max_depth": max_depth,
                    "solution_length": len(path),
        ...
        for move_name in MOVES:
            ...
            if new_state:
                neighbors.append((normalize_state(new_state), move_name))
        ...
        for neighbor, move_name in neighbors:
            # Chỉ cập nhật belief_state khi cần thiết
            temp_belief = update_belief(belief_state, neighbor)
            if temp_belief:
                # Giới hạn kích thước belief_state
                if len(temp_belief) > max_belief_size:
                    # Sắp xếp theo khoảng cách Manhattan và giữ lại các trạng thái tốt nhất
                    temp_belief.sort(key=lambda s: manhattan_distance(s, goal))
                    temp_belief = temp_belief[:max_belief_size]
                avg_manhattan = sum(manhattan_distance(s, goal) for s in temp_belief) / len(temp_belief)
                if avg_manhattan < current_best_score:
                    current_best_score = avg_manhattan
        ...
        # Kiểm tra tiến trình để dừng nếu không cải thiện
        ...
        if steps_without_improvement > 50:  # Dừng nếu không cải thiện sau 50 bước
            statistics.update({
                "nodes_expanded": nodes_expanded,
                "max_depth": max_depth,
                "solution_length": len(path),
        ...
        next_state, move_name = best_neighbor
        path.append(next_state)
        belief_state = update_belief(belief_state, next_state)
...
def run_algorithm(initial_state, sorted_state, buttons, algorithm):
    ...
    algorithm_map = {
        ...
        "Q-Learning": q_learning  # Thêm Q-Learning vào ánh xạ
    }

    # ... (Giữ nguyên phần còn lại của run_algorithm, chỉ đảm bảo nó hoạt động với Q-Learning)
